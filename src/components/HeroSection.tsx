...
const HeroSection = () => {
  return (
    <section className="relative flex items-center justify-center min-h-[90vh] overflow-hidden bg-black">
      ...
      {/* --- EFECTO DE HUMO DE FONDO (CSS PURO) --- */}
      <div className="absolute inset-0 z-0 opacity-40">
        {/* Capa 1: Humo morado lento */}
        <div className="absolute top-[-50%] left-[-50%] w-[200%] h-[200%] bg-[radial-gradient(circle,rgba(124,58,237,0.3)_0%,transparent_50%)] animate-[spin_25s_linear_infinite]" />
        {/* Capa 2: Niebla gris movimiento contrario */}
        <div className="absolute bottom-[-50%] right-[-50%] w-[200%] h-[200%] bg-[radial-gradient(circle,rgba(255,255,255,0.1)_0%,transparent_40%)] animate-[spin_35s_linear_infinite_reverse]" />
      </div>
      ...
      <div className="relative z-20 container text-center px-4 space-y-8 pt-20">
        ...
        <div className="inline-block animate-fade-in-up opacity-0 [animation-delay:100ms]">
          <span className="py-1 px-3 rounded-full border border-primary/30 bg-primary/10 text-primary text-sm font-medium tracking-wide">
            EST. 2024 • LIMA, PERÚ
          </span>
        </div>
        ...
        <h1 className="font-heading text-6xl md:text-8xl lg:text-9xl font-black tracking-tighter text-white opacity-0 animate-fade-in-up [animation-delay:300ms]">
          MR. <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-purple-400 to-primary animate-gradient-x">HUMO</span>
        </h1>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4 opacity-0 animate-fade-in-up [animation-delay:700ms]">
          <Link
            to="/catalogo"
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-full bg-primary px-8 py-4 font-bold text-white transition-all hover:bg-primary/90 hover:scale-105 shadow-[0_0_20px_rgba(124,58,237,0.5)]"
          >
            Ver Catálogo
          </Link>
          <Link
